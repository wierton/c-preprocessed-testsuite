...
int match (float _Complex c, float _Complex z)
{
  ...
  if (__builtin_fabsf (rz) > 1.17549435082228750796873653722224568e-38F)
    {
    ...
    {
      rerr = __builtin_fabsf (rz - rc) / 1.17549435082228750796873653722224568e-38F;
    }

  if (__builtin_fabsf (iz) > 1.17549435082228750796873653722224568e-38F)
    {
    ...
    {
      ierr = __builtin_fabsf (iz - ic) / 1.17549435082228750796873653722224568e-38F;
    }
  rmax = __builtin_fmaxf(rerr, ierr);
  biterr = 0;
  if ( rmax != 0.0)
    {
      biterr = ilogbf (rmax) + 24 + 1;
    }
